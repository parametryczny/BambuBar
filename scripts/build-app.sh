...
cp ".build/debug/BambuBar" "$APP_PATH/Contents/MacOS/BambuBar"
chmod +x "$APP_PATH/Contents/MacOS/BambuBar"
...
if [[ -n "$SIGN_HASH" ]]; then
    codesign --force --sign "$SIGN_HASH" --identifier "$BUNDLE_ID" --entitlements "Resources/BambuBar.entitlements" "$APP_PATH"
else
    ...
    codesign --force --sign - --identifier "$BUNDLE_ID" --requirements "$APP_REQUIREMENT" --entitlements "Resources/BambuBar.entitlements" "$APP_PATH"
fi
xattr -cr "$APP_PATH"
mkdir -p "$OUTPUT_PATH"
